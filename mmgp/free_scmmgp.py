...
class FreeSCMMGP(object):
    """ 
    Main class representing the sparse model where latent functions may covary
        with Kronecker-structured, freely parameterised prior covariance
        (full or sparse)
    ...
    def __init__(self,
        ...
        self.num_block = len(block_struct)
        ...
        # Prior parameters and inducing points
        ...
        # raw parameters for free cross-function covar in prior - full lower triangular for max block size
        # (trimmed for ragged block sizes internally)
        init_priorvec = np.zeros([self.num_block] +
                                [int(x) for x in util.tri_vec_shape(len(max(self.block_struct, key=len)))], dtype=np.float32)
        self.raw_priorlink_covars = tf.Variable(init_priorvec)
        ...
        self.nelbo, self.predictions, self.general_nlpd = self._build_graph(self.raw_weights,
                                                                        ...
                                                                        self.raw_priorlink_covars, # new in freesparse
    ...
    def _print_state(self, data, test, loss, num_train, batch_size, fit_stime, batch_stime):
        ...
        with open("log_params.csv", 'a', newline='') as f:
            csv.writer(f).writerow(kout)
        #if self.raw_priorlink_covars:
            #priorcov_out = self.session.run(self.raw_priorlink_covars)
            #np.savez_compressed("raw_priorlink_covars.npz", *priorcov_out)
    ...
    def _build_graph(self, raw_weights, raw_means, raw_covars, raw_link_covars, raw_priorlink_covars, 
                    raw_inducing_inputs, train_inputs, train_outputs, num_train, test_inputs, test_outputs):
        # normalise weights
        ...
        mat = util.vec_to_tri(raw_priorlink_covars) #creates lower diag mats by row ie r so R x max(Qr) x max(Qr)
        diag_mat = tf.matrix_diag(tf.matrix_diag_part(mat))
        exp_diag_mat = tf.matrix_diag(tf.exp(tf.matrix_diag_part(mat)))
        ...
        for r in range(self.num_block):
            if len(self.block_struct[r]) == 1:  # leave as dummy value == 1.0
                ...
                kernlink_chol[r] = mats_in[r, :len(self.block_struct[r]), :len(self.block_struct[r])]
