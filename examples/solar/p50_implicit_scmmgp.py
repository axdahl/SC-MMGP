# -*- coding: utf-8 -*-
"""
Script to execute example implicit sparse covarying MMGP regression forecasting model.
...
Data for example:
 ...
 - Xtr[:, 100] time index
 - link inputs is a 50x2 array (link inputs repeated for every group)
...
Current Settings (sparse covarying mmgp model with sparse Kronecker posterior):
    DIAG_POST = False
    SPARSE_PRIOR = True  # set True for equivalent sparse mmgp model
    SPARSE_POST = True
...
dlinkfile = 'p50_linkinputsarray.pickle'
outdir = '/experiments/results/p50_implicit_scmmgp'
...
for i in range(output_dim):
    ...
    row_0 = row.pop(i)     # bring diag to first position
...
# link kernel
klink_rows = [kernels.WaveletSlice(2, active_dims=[0,1], input_scaling=IS_ARD) for i in range(output_dim) ]
...
m.fit(data, o, loo_steps=0,  var_steps = VAR_STEPS, epochs = EPOCHS, batch_size = BATCH_SIZE, display_step=DISPLAY_STEP,
