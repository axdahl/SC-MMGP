...
Current Settings (free sc-mmgp model with diagonal posterior):
    DIAG_POST = True
    SPARSE_PRIOR = False  # set True for equivalent sparse free sc-mmgp model
    SPARSE_POST = True
...
outdir = '/experiments/results/p25_nonsparse_free_scmmgp_diagonal'
...
# order of block_struct is rows, diagonals, node functions
# lists required: block_struct,  kern
...
m = mmgp.FreeSCMMGP(output_dim, likelihood, kern, block_struct, Z,
...
# initialise losses and logging, switch to output directory
...
m.fit(data, o, loo_steps=0,  var_steps = VAR_STEPS, epochs = EPOCHS, batch_size = BATCH_SIZE, display_step=DISPLAY_STEP, test = test,
