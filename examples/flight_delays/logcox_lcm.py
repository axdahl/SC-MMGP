# -*- coding: utf-8 -*-
"""
Script to execute example linear coregional model with Poisson likelihood.
...
outdir = '/experiments/results/logcox_lcm'
...
save_nlpds = True # If True saves samples of nlpds for n,p,s
...
# cluster IDs
loc_r= Xtr[:, len(commonfeats):]
loc_clustID = feat_clust(np.transpose(loc_r), node_dim)

print("dataset created")
likelihood = likelihoods.CoregionLogCox(output_dim, node_dim,
    0.5, offset=0.05, predict_samples=PRED_SAMPLES) # output_dim, latfunc_dim, weight, offset, predsamples

# define active dims for each airport
...
if output_dim!=node_dim:
    for i in range(node_dim):
        ...
        kf[i] = kernels.CompositeKernel('add',
                [kernels.RadialBasisSlice(len(cl_dims), active_dims=cl_dims, std_dev = 0.5,
                lengthscale=0.5, white = 0.01, input_scaling = IS_ARD),
                kernels.PeriodicSlice(1, active_dims=[0], lengthscale=0.0, std_dev=0.5, period = 1.0)] )
else:
    # Qg = P configuration
    for i in range(output_dim):
        kf[i] = kernels.CompositeKernel('add',
                [kernels.RadialBasisSlice(locfeat_dim, active_dims=locfeat_active_dims[i], std_dev = 0.5,
                lengthscale=0.5, white = 0.01, input_scaling = IS_ARD),
                kernels.PeriodicSlice(1, active_dims=[0], lengthscale=0.0, std_dev=0.5, period = 1.0)] )

# extra configs - all features in each node
kf = [kernels.CompositeKernel('add',
                [kernels.RadialBasisSlice(Xtr.shape[1], active_dims=list(range(Xtr.shape[1])), std_dev = 0.5,
                lengthscale=0.5, white = 0.01, input_scaling = IS_ARD),
                kernels.PeriodicSlice(1, active_dims=[0], lengthscale=0.0, std_dev=0.5, period = 1.0)] )
                for i in range(node_dim)]
# coregional
